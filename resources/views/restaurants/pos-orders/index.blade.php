            ...
            <div class="col-md-12">
                <div class="card">
                    {{-- <div class="card-header">
                        <form action="{{ route('restaurants.pos-orders.index') }}" method="get">
                            <div class="row mb-2">
                            ...
                            <tbody>
                                @if (count($orders) > 0)
                                    @foreach ($orders as $order)
                                        <tr>
                                            <td><a
                                                    href="{{ route('restaurants.pos-orders.show', $order->id) }}">{{ $order->id }}</a>
                                            </td>
                                            ...
                                            <td>
                                                <a href="{{ route('restaurants.pos-orders.show', $order->id) }}"
                                                    class="btn btn-primary btn-sm">View</a>
                                                <form action="{{ route('restaurants.pos-orders.destroy', $order->id) }}"
